...
const StudentForm = ({ student, onClose, onSuccess }: StudentFormProps) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.rfid_tag_number.trim())
      newErrors.rfid_tag_number = "RFID tag number is required";
          ...
          <CardContent className="p-6 space-y-6">
                      ...
                      {profilePictureFile ? (
                        <>
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Size:{" "}
                            {profilePictureFile.size / 1024 > 1024
                              ? (
                                  profilePictureFile.size /
                                  (1024 * 1024)
                                ).toFixed(2) + " MB"
                              : (profilePictureFile.size / 1024).toFixed(2) +
                                " KB"}
                          </p>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-slate-900 flex items-center gap-2">
               ...
               <p className="text-sm text-amber-600 bg-amber-50 px-3 py-2 rounded mt-1">
                <span className="font-semibold">
                  <TriangleAlert className="w-4 h-4 inline mr-1" /> Note:
                </span>{" "}
                Before tapping the card, ensure the RFID textbox is active. The
                data will be saved automatically once the card is tapped.
              </p>

              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-2">
                  <Label
                    htmlFor="rfid_tag_number"
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Tap the unique RFID tag number assigned to this student.
                  </p>
                </div>
              </div>
            </div>
